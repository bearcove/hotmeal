...
/// Convert a Rust identifier to an HTML element name (lowercase with hyphens).
fn to_element_name(name: &str) -> Cow<'_, str> {
    // Simple conversion: lowercase, no changes needed for most HTML element names
    // For PascalCase like "CustomElement", convert to "custom-element"
    ...
    if result == name.to_lowercase() && !result.contains('-') {
        Cow::Borrowed(name)
    } else {
        Cow::Owned(result)
    }
