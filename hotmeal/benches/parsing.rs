...
fn parse_small(bencher: Bencher) {
    bencher.bench_local(|| {
        let doc = hotmeal::parse_document(black_box(SMALL_HTML));
        black_box(doc);
    });
}
...
fn parse_medium(bencher: Bencher) {
    bencher.bench_local(|| {
        let doc = hotmeal::parse_document(black_box(MEDIUM_HTML));
        black_box(doc);
    });
}
...
fn parse_large(bencher: Bencher) {
    bencher.bench_local(|| {
        let doc = hotmeal::parse_document(black_box(LARGE_HTML));
        black_box(doc);
    });
}
...
fn parse_xlarge(bencher: Bencher) {
    bencher.bench_local(|| {
        let doc = hotmeal::parse_document(black_box(XLARGE_HTML));
        black_box(doc);
    });
}
