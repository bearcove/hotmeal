...
/// Regression test for OOM in compute_path with deeply nested footers
/// minimized-from-194e3727bd7233ffeb1333b97782d2fd064955ac (113 bytes)
///
/// The issue: deeply nested structures could cause compute_path to run
/// unboundedly, leading to 2GB allocation attempts and OOM.
#[test]
fn test_deeply_nested_footers_oom() {
    // Old: Footer(Footer(Address, Footer(Footer(Footer, Footer(Footer(Footer(Text)))))))
    // New: Footer(Footer(Footer(Footer(Footer(Footer(Footer(Canvas)))))))
    let old_html = t(
        r#"<html><body><footer><footer><address></address><footer><footer><footer></footer><footer><footer><footer></footer></footer></footer></footer></footer></footer></footer></body></html>"#,
    );
    let new_html = t(
        r#"<html><body><footer><footer><footer><footer><footer><footer><footer><canvas></canvas></footer></footer></footer></footer></footer></footer></footer></body></html>"#,
    );

    trace!(?old_html, "Parsing old");
    let doc_a = hotmeal::parse(&old_html);
    trace!(doc_a_html = %doc_a.to_html(), "doc_a HTML");

    trace!(?new_html, "Parsing new");
    let doc_b = hotmeal::parse(&new_html);
    trace!(doc_b_html = %doc_b.to_html(), "doc_b HTML");
